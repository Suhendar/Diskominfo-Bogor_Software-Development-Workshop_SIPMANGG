...
const initializeDatabase = async (retries = 3) => {
  try {
    ...
    // Fallback to SQLite if Postgres is unreachable after retries
    try {
      console.warn(
        "PostgreSQL unreachable after retries. Falling back to SQLite for local development."
      );
      const dbPath = path.join(__dirname, "..", "database.sqlite");
      sequelize = new Sequelize({
        dialect: "sqlite",
      ...
      // Update exported reference so other modules use the new instance
      module.exports.sequelize = sequelize;

      await sequelize.authenticate();
      console.log("SQLite fallback connection established successfully.");
      await sequelize.sync({ alter: true });
      console.log("SQLite models synchronized.");
      return;
    } catch (fallbackError) {
      console.error("SQLite fallback failed:", fallbackError);
...
module.exports = {
  ...
  NotificationLog,
  Admin,
  // Export Admin if defined below
  // Admin will be assigned after its declaration
  initializeDatabase,
};
